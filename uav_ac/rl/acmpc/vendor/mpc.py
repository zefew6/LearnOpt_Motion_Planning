...
class MPC(Module):
    ...
    def forward(self, x_init, cost, dx):
        ...
        for i in range(self.lqr_iter):
            ...
            if best is None:
                ...
                for j in range(n_batch):
                    if (best['full_du_norm'][j] >= self.eps and
                            costs[j] <= best['costs'][j] + self.best_cost_eps):
                        n_not_improved = 0
        ...
        self.last_residual = full_du_norm.detach().clone()
        self.last_iterations = i + 1

        if not self.backprop:
            return x.detach(), u.detach(), best['costs'].detach()
