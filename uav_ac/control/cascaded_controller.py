...
class CascadedController:
    """
    Cascaded controller from the paper:
    https://www.dynsyslab.org/wp-content/papercite-data/pdf/lupashin-mech14.pdf

    This class now implements the generic trajectory-controller interface:
    ``step(quad, reference) -> ControlCommand``.  The internal cascaded
    structure remains private to this controller, so the trajectory scheduler
    does not need to know about altitude, attitude, or body-rate loops.
    """
    ...
    def __init__(self, g: float, dt: float):
        ...
        :param dt: time step of the outer control/reference loop
        """
        self.g = g
        self.dt = dt
    ...
    def reset(self) -> None:
        ...
        self._thrust_cmd = 0.0
        self._pqr_cmd.fill(0.0)

    def step(self, quad, reference: TrajectoryReference) -> ControlCommand:
        """Execute one cascaded-control cycle.

        The position/altitude/reduced-attitude outer loop is updated only when
        the active trajectory sample changes.  The body-rate loop is evaluated
        every call, preserving the original fast-inner/slow-outer behavior.
        """
        if (reference.index != self._last_reference_index
                or (reference.is_terminal and reference.is_control_tick)):
            self._update_outer_loop(quad, reference.current)
            self._last_reference_index = reference.index

        moment_cmd = self.body_rate_controller(
            quad,
            self._pqr_cmd,
            quad.kp_p,
            quad.kp_q,
        ...
        return ControlCommand(self._thrust_cmd, moment_cmd)

    def _update_outer_loop(self, quad, target: np.ndarray) -> None:
        """Update altitude, lateral, and reduced-attitude commands."""
        rotation = quad.R()
        thrust_cmd = self.altitude(
            quad,
            target[[2, 5, 8]],
            rotation,
            quad.kp_z,
        ...
        self._thrust_cmd = thrust_cmd
        self._pqr_cmd = self.reduced_attitude(
            quad,
            bxy_cmd,
            target[9],
            rotation,
    ...
    def altitude(self, quad, des_z, rot_mat, kp_z, kd_z, ki_z):
        ...
        self.integral_error = np.clip(
            self.integral_error,
            -CascadedController.INTEGRAL_ERROR_LIMIT,
            CascadedController.INTEGRAL_ERROR_LIMIT,
        )

        acc_z = CascadedController._pid(
            kp_z, kd_z, ki_z, error, error_dot, self.integral_error, des_z[2]
        ) - self.g
    ...
    def lateral(self, quad, des_x, des_y, thrust_cmd, kp_xy, kd_xy):
        ...
        acc_cmd = CascadedController._pd(
            lateral_p_gain, lateral_d_gain, pos_err, vel_err, ff_acc
        )
    ...
    def body_rate_controller(self, quad, pqr_cmd, kp_p, kp_q, kp_r):
        ...
        moment_cmd = inertia * kp_pqr * (pqr_cmd - pqr_actual)
        moment_cmd += np.cross(pqr_actual, inertia * pqr_actual)
        return moment_cmd
    ...
    def roll_pitch_controller(self, bxy_cmd, rot_mat, kp_roll, kp_pitch):
        ...
        return np.matmul(rot_mat1, b_xy_cmd_dot.T)
    ...
    def yaw_controller(self, quad, psi_des, kp_yaw, q_cmd=0.0):
        ...
        return (
            yaw_rate_cmd * np.cos(quad.theta) - q_cmd * np.sin(quad.phi)
        ) / np.cos(quad.phi)
    ...
    def wrap_to_2pi(angle):
        """Map an angle to the range [0, 2*pi)."""
