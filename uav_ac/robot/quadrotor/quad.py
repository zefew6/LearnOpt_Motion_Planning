...
class Quad:
    ...
    def set_propeller_speed(self, thrust_cmd: float, moment_cmd: np.ndarray):
        ...
        """
        self.command_propeller_speed(thrust_cmd, moment_cmd)
        self.update_motor_response()

    def command_propeller_speed(self, thrust_cmd: float, moment_cmd: np.ndarray) -> None:
        """Set target speeds without advancing motor dynamics."""
        rotor_forces = self._allocate_rotor_forces(thrust_cmd, moment_cmd)
        self.omega_command = np.sqrt(rotor_forces / self.kf)

    def update_motor_response(self) -> None:
        """Advance first-order motor dynamics by one physics timestep."""
