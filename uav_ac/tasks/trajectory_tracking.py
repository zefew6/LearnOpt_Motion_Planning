...
from uav_ac.control.trajectory_controller import TrajectoryReference, TrajectoryTrackingController
...
def validate_tracking_scene(simulation: MujocoSimulation) -> None:
    """Validate the mission fields required by the existing tracking workflow."""
    if simulation.goal_position is None:
        raise ValueError("trajectory tracking requires a goal site")
    if len(simulation.mission_waypoints) < 3:
        raise ValueError("trajectory tracking requires at least one mandatory waypoint")
    bounds = simulation.space_limits
    if (bounds is None or not np.all(np.isfinite(bounds))
            or np.any(bounds[0] >= bounds[1])):
        raise ValueError("trajectory tracking requires finite ordered planning_bounds")
...
class MujocoTrajectoryTrackingEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def __init__(
            ...
            *,
            ...
            fixed_wind: GustingCrosswind | None = None,
            success_position_error: float = SUCCESS_POSITION_ERROR,
            wind_seed: int | None = None,
    ):
        super().__init__()
        if not np.isfinite(success_position_error) or success_position_error <= 0.0:
            raise ValueError("success_position_error must be positive and finite")
        if fixed_wind is not None and wind_config is not None:
            raise ValueError("fixed_wind and wind_config are mutually exclusive")
        self.fixed_wind = fixed_wind
        self.success_position_error = float(success_position_error)
        ...
        validate_tracking_scene(self.simulation)
    ...
    def reset(
        ...
        if seed is not None:
            self._wind_rng = np.random.default_rng(
                self.wind_seed if self.wind_seed is not None else seed)
    ...
    def reference(self) -> TrajectoryReference:
        """Current reference at the beginning of the next control interval."""
        return self._reference()

    def step_controller(self, controller: TrajectoryTrackingController):
        """Run feedback every physics substep, then score one tracking interval.

        Only the first substep is a control tick; the reference index advances
        once per interval. The caller resets its controller between episodes.
        Reward action penalties describe the final substep's normalized wrench.
        """
        return self._advance(None, self._previous_action.copy(), None, controller)

    def _advance(self, action, previous_action, command, controller=None):
        from uav_ac.rl.acmpc.metrics import allocation_scale
        from uav_ac.rl.acmpc.metrics import command_to_action
        actuator_scale = None if controller is not None else allocation_scale(command, self.quad)

        finite_state = True
        for substep in range(self.steps_per_action):
            if controller is not None:
                command = controller.step(
                    self.quad, replace(self.reference, is_control_tick=substep == 0))
                action = command_to_action(command, self.quad)
    ...
    def _reset_wind(self, options: dict[str, Any]) -> None:
        ...
        if self.fixed_wind is not None:
            self._wind_enabled = bool(options.get("wind_enabled", True))
            self._wind = self.fixed_wind if self._wind_enabled else None
            return
        if self.wind_config is None:
            self._wind_enabled = False
            self._wind = None
        ...
        enabled = bool(options.get(
            "wind_enabled",
            self._wind_rng.random() < self.wind_config.probability,
        ))
        ...
        self._wind = (
            sample_gusting_crosswind(self._wind_rng, self.wind_config, scale=scale)
    ...
    def _is_success(self, metrics: dict[str, float]) -> bool:
        return bool(
            ...
            and metrics["position_error"] < self.success_position_error
