...
def test_mujoco_simulation_should_apply_persistent_external_world_force(simulation):
    ...
    simulation.set_external_force_world(force_world)

    # Act
    simulation._apply_rotor_forces()

    # Assert
    assert simulation.data.qfrc_applied[:3] == pytest.approx(force_world)


@pytest.mark.parametrize("force", [np.zeros(2), np.array([0.0, np.nan, 0.0])])
def test_set_external_force_world_should_reject_invalid_force(simulation, force):
    # Arrange
    # Act
    def set_invalid_force():
        simulation.set_external_force_world(force)

    # Assert
    with pytest.raises(ValueError, match="external_force_world"):
        set_invalid_force()


class FakeViewerHandle:
    def __init__(self, simulation, step_count=2, reset_after_sync=None):
        self.simulation = simulation
        self.cam = mujoco.MjvCamera()
        self.remaining_steps = step_count
        self.reset_after_sync = reset_after_sync
        self.sync_count = 0
    ...
    def sync(self):
        self.sync_count += 1
        if self.sync_count == self.reset_after_sync:
            mujoco.mj_resetData(self.simulation.model, self.simulation.data)
...
def test_run_interactive_should_initialize_camera_from_model_and_step_once_per_cycle(simulation):
    # Arrange
    control_times = []
    ...
    viewer_handle = FakeViewerHandle(simulation)

    # Act
    with patch("mujoco.viewer.launch_passive", return_value=viewer_handle):
        with patch("time.sleep"):
            simulation.run_interactive(record_control_time)

    # Assert
    assert control_times == pytest.approx([0.0, simulation.model.opt.timestep])
    assert viewer_handle.cam.lookat == pytest.approx(simulation.model.stat.center)
    assert viewer_handle.cam.distance == pytest.approx(1.5 * simulation.model.stat.extent)
    assert viewer_handle.cam.azimuth == pytest.approx(simulation.model.vis.global_.azimuth)
    assert viewer_handle.cam.elevation == pytest.approx(simulation.model.vis.global_.elevation)
...
def test_run_interactive_should_reset_runtime_when_viewer_rewinds_time(simulation):
    ...
    viewer_handle = FakeViewerHandle(simulation, step_count=3, reset_after_sync=3)

    def record_control_time_and_dirty_runtime():
        record_control_time()
        if simulation.data.time == simulation.model.opt.timestep:
    ...
    with patch("mujoco.viewer.launch_passive", return_value=viewer_handle):
        with patch("time.sleep"):
            simulation.run_interactive(record_control_time_and_dirty_runtime, record_reset_time)
